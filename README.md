...
- A v0-built prototype without auth, persistence, or logs likely needs production readiness basics first.
...
Because of that, the prompt tells the model to use a URL only as product or business context, not as proof of hidden infrastructure.
...
2. Next.js frontend sends intake to /api/analyze.
3. Server-side API route reads the JSON body.
4. Route builds the Startup Advisor prompt.
5. Vercel AI SDK calls OpenAI with generateObject.
6. Zod schema forces structured output.
7. API route returns JSON.
8. Frontend stores result in state.
9. Recommendation report renders in the UI.
...
This file is marked with "use client" because it uses React state and click handlers.
...
The prompt says: think like a Vercel Startup Solutions Architect.

The schema says: return an object with predictable fields the frontend can render.

This makes the app easier to reason about, easier to debug, and easier to present.

## Vercel platform notes

The app uses a Next.js API route at app/api/analyze/route.ts.
...
The route includes export const maxDuration = 60, which gives the AI analysis endpoint more tolerance for slower model responses before timing out.

## Why the model call happens server-side

The model call happens inside app/api/analyze/route.ts, not directly inside app/page.tsx.
...
The app uses generateObject from the Vercel AI SDK because it lets the route request structured output that matches the Zod schema.
...
For example, the schema requires a primaryRecommendation object with product, whyNow, and businessValue fields.
...
1. Direct OpenAI calls plus rising model spend plus no fallback  
   Expected: AI Gateway

2. v0 prototype plus no auth, database, logs, or rollback confidence  
   Expected: Production readiness with Functions, environment variables, and Observability

3. Frontend on Vercel plus Railway worker handling background jobs  
   Expected: Workflows or Fluid Compute

4. AI-generated or user-generated code needs safe execution  
   Expected: Sandbox

5. Public AI endpoint plus bot traffic or runaway usage risk  
...
- GitHub or repo analysis to detect framework usage, direct model imports, workers, and security gaps
...
The production version would combine public website context, founder-provided context, repo analysis, Vercel telemetry, and AI Gateway usage data.
...
Production on Vercel requires adding the environment variable OPENAI_API_KEY in the Vercel dashboard.
...
The project's .gitignore should include .env*.

Before committing, check git status and make sure .env.local is not listed under staged files.
...
- page.tsx collects input, calls /api/analyze, and renders result.
- route.ts runs server-side, calls OpenAI through the AI SDK, and returns JSON.
- prompts.ts defines the Startup Advisor reasoning framework.
- schema.ts defines the structured output contract.
- README.md explains the product, architecture, tradeoffs, evals, and production path.
